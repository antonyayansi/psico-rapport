    ...
    <div v-else dir="ltr"
        ...
        <div
            ...
            <nav v-if="!['Home', 'Login', 'Onboarding', 'Consent'].includes($route.name)"
                class="flex-none bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-t border-slate-200/50 dark:border-slate-800 px-2 pt-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] flex justify-between items-center text-[0.68rem] font-medium z-20">
                <button @click="router.push('/tracking')"
                    :class="$route.name === 'Tracking' ? 'text-green-700 dark:text-slate-100' : 'text-slate-400'"
                ...
                <button @click="router.push('/uwu')"
                    :class="$route.name === 'UwuChat' ? 'text-green-700 dark:text-slate-100' : 'text-slate-400'"
                    class="flex flex-col items-center p-1.5 transition-colors flex-1">
                    <svg class="w-5 h-5 mb-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75"
                ...
                <button @click="router.push('/pet')"
                    :class="$route.name === 'Pet' ? 'text-green-700 dark:text-slate-100' : 'text-slate-400'"
                    class="flex flex-col items-center p-1.5 transition-colors flex-1">
                    <svg class="w-5 h-5 mb-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75"
                ...
                <button @click="router.push('/community')"
                    :class="$route.name === 'Community' ? 'text-green-700 dark:text-slate-100' : 'text-slate-400'"
                ...
                <button @click="router.push('/therapists')"
                    :class="['Therapists', 'TherapistProfile', 'TherapistRegister', 'Admin'].includes($route.name) ? 'text-green-700 dark:text-slate-100' : 'text-slate-400'"
                    class="flex flex-col items-center p-1.5 transition-colors flex-1">
                    <svg class="w-5 h-5 mb-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75"
                ...
                <button @click="router.push('/stats')"
                    :class="$route.name === 'Stats' ? 'text-green-700 dark:text-slate-100' : 'text-slate-400'"
                    class="flex flex-col items-center p-1.5 transition-colors flex-1">
                    <svg class="w-5 h-5 mb-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75"
